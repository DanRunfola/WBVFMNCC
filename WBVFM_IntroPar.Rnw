...
First, we truncate the data set to cover World Bank projects from 2000 to 2010 due to limitations of our ancillary information, resulting in a total of \Sexpr{active_proj_count} World Bank project locations.
...
This is conducted following a three-stage process which is repeated for every large scale World Bank project, resulting in \Sexpr{analysis_proj_count} total models.
...
\par
For each of these \Sexpr{analysis_proj_count} subpopulations, we match control and treatment cases on the basis of the propensity scores estimated in eq.\ref{EQpropensity}, following a nearest-neighbor matching strategy.  
