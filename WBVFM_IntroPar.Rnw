...
\section{Abstract}
 ...
 We apply a geographically explicit matching method (\cite{andam_measuring_2008}) to estimate the impact of World Bank projects implemented between 2000 and 2010 on sequestered carbon, using a novel and publicly available data set of \Sexpr{proj_count} World Bank project locations. 
 Considering only carbon sequestered due to fluctuations in vegetative biomass caused by World Bank projects, we estimate a global net positive increase in sequestration totaling \Sexpr{tonnes_sequestered} tonnes. 
...
points(coordinates(uni_spdf_LL ), col="red", pch=20, cex=0.05)
@
...
\par
We find that while the overall impact of World Bank projects on carbon sequestration appears to be positive (an increase of \Sexpr{tonnes_sequestered} sequestered tonnes), considerable temporal and spatial variation exists in these impacts.
...
First, we truncate the data set to cover World Bank projects from 2000 to 2010 due to limitations of our ancillary information, resulting in a total of \Sexpr{proj_count} World Bank project locations.
...
This is conducted following a three-stage process which is repeated for every unit of analysis, resulting in \Sexpr{proj_count} total models.
...
\par
For each of these \Sexpr{proj_count} sub populations, we match control and treatment cases on the basis of the propensity scores estimated in eq.\ref{EQpropensity}, following a nearest-neighbor matching strategy.  
...
This results in \Sexpr{proj_est_count} project locations which have adequate matches for estimation, or \Sexpr{round(proj_est_count/as.numeric(proj_count),2)*100}\% of all project locations. 
