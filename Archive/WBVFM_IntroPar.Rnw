...
We apply a geographically explicit matching method (\cite{andam_measuring_2008}) to estimate the relative impact of large scale World Bank projects implemented between 2000 and 2010 on sequestered carbon, using a novel and publicly available data set of \Sexpr{proj_count} World Bank project locations. 
...
<<WLocs, echo=FALSE, fig.width=3, fig.height=2>>=
...
First, we truncate the data set to cover World Bank projects from 2000 to 2010 due to limitations of our ancillary information, resulting in a total of \Sexpr{active_proj_count} World Bank project locations, of which \Sexpr{pc1_proj_count} have exact location data (precision code 1).
...
The projects locations are then subdivided into six different groups based on project sectors - Health, Environment, Education, Industrial, Infrastructure, and Other. Indvidual project locations and associated commitment amounts are split based on project sector to produce sector independent locations which allows for analysis of sector groups. 
...
This is conducted following a three-stage process which is repeated for every large scale World Bank project location, resulting in a model for each of the sector independent project locations.
...
<<Fig1, echo=FALSE, fig.height=5, fig.width=6>>=
...
This results in \Sexpr{proj_est_count_high} locations which have adequate matches for estimation, or \Sexpr{round(proj_est_count_high/as.numeric(analysis_proj_count),2)*100}\% of all large scale projects' locations. 
...
The impacts estimated for each of these locations (\begin{math}\theta\end{math} in eq. \ref{EQgwr}) are entered in to the fixed-effect model derived following eq. \ref{EQcarb} (a total of 60 fixed effects estimates are derived in this model, representing all ecofloristic zones which contained World Bank projects).
...
We chose to contrast high-dollar value World Bank projects to low-dollar value World Bank projects but were unable to match within the same sector due to insufficient matches based on the available data. Instead, the sector group was included as a fixed effect in the propensity score model. This approach is a substitute for sectors matching, i.e., comparing  ``apples to apples", intended to reduce omission bias.
