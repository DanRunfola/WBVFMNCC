...
foreach (v=1:cores) %dopar% {
# foreach (v=1:1) %do% {
        
    print(v)
    
    output_base <- paste(base_dir, '/wb_pc_model_04/carbon_output/carbon_par_medium/carbon_',k,'_',v,'_', sep='')
    writeLines(c(''), paste(output_base,'log.txt', sep=""))
    
    range_start <- (v-1) * ceiling(nrow(tmp_psmRes@data) / cores) + 1
    range_end <- v * ceiling(nrow(tmp_psmRes@data) / cores) 
    range_end <- min(range_end, nrow(tmp_psmRes@data))
    ...
    output_data <- output_data[range_start:range_end,]
