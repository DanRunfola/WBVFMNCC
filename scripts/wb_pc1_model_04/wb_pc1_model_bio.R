...
shpfile = paste(base_dir, "/wb_pc1_model_04/wb_pc1_data_sectors_short_bio.geojson", sep="")
...
pModelMax <- "iba_statescore ~ TrtBin + PSM_match_ID + gpw3_2000e + alp4_1999e +
am50_e + sslp_e + selv_e + dari_e + droa_e + 
pre_avg_NDVI_max + pre_avg_temp_mean + pre_avg_precip_mean + post_avg_temp_mean + post_avg_precip_mean +
pre_trend_temp_mean + pre_trend_precip_mean + post_trend_temp_mean + post_trend_precip_mean + 
sector_group + sector_group:TrtBin + post_avg_NDVI_max"
...
# for ( k in c('low_trtbin', 'medium_trtbin', 'high_trtbin') ) {
    ...
    output_base <- paste(base_dir, '/wb_pc_model_04/biodiversity_output/bio_par_test_1_',k,'_', sep='')
